...
\subsection{Procesy}
Nedílnou součástí služby jsou také její procesy, které slouží k jejímu úspěšnému provozování a rozvoji. Procesy, které jsou v rámci datového skladu využívány, mají za úkol zajištění její dostupnosti a postupné zlepšování. Za tímto účelem je tedy již přímo v zakládací listině částečně definován proces sloužící k žádosti o nový usecase, dále je definovat proces pro nahlášení nefunkční služby. Dalším plánovaným procesem je pak řadě proces týkající se aktivní akvizice nový usecasů prostřednictví nabízení datového skladu jako služby přímo koncovým uživatelům, který má za úkol zvýšit množství uživatelů využívající datový sklad a tím dosáhnout splnění zadaného KPI. Všechny tyto procesy jsou detailněji popsány níže.
\begin{itemize}
  \item Žádost o nový usecase - Proces požadavku na tvorbu nového reportu má několik fází, které jsou vyobrazeny na obr. \ref{fig:new_report}. 
        ...
        \end{itemize}
  \item Nahlášení nefunkční služby - Pro nahlášení nefunkční služby slouží formulář uvedený na stránkách datového skladu v katalogu služeb Masarykovy univerzity, tento proces kromě samotného týmu datového skladu zahrnuje i centrální service desk Masarykovy univerzity. Celý vyobrazen na schématu \ref{fig:service_fail}
        \begin{itemize}
            \item Pokud uživatel objeví problém, který souvisí s datovým skladem, může k nahlášení tohoto problému využít zmíněný formulář na webových stránkách datového skladu MUNI. Po nahlášení dojde k vytvoření tzv. tiketu v interním systému centrálního service desku Masarykovy univerzity. 
            \item Service desk následně zkontroluje stav konkrétního usecasu a v případě potvrzení problému nejprve provede kontrolu, zdali se nejedná o plánovaný výpadek služby, který může být způsoben například aktualizací daného reportu nebo samotné služby datového skladu. 
            ...
            \item Tým datového skladu následně posoudí závažnost reportovaného problému a jejím základě buď problém vyřeší své pomocí a nebo v případě velké komplikovanost kontaktuje dodavatele datového skladu v jehož součinnosti zjednají nápravu. 
        \end{itemize}
