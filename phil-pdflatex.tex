...
Tyto dva zmíněné druhy tabulek může doplňovat ještě typ třetí, který se nazývá můstkový a slouží jako propojovací prvek mezi fakty a dimenzemi.

V praxi běžně nastává situace, kdy může dojít k propojení více datových hvězd, čímž dojde ke vzniku datového modelu zvaného souhvězdí. K této situaci typicky dochází v případech, kdy spolu jednotlivé fakty úzce souvisí, např. prodej - platba. V tomto případě jsou obě tabulky samostatné datové hvězdy, ale jsou spolu vzájemně propojeny.\footnote{doplnit obr}
...
\paragraph{Aktivní nabízení služby datového skladu} - Proces týkající se rozvoje služby. Cílem procesu je pro-aktivní přístup k rozšiřování uživatelských usecasů, postavených na službě datového skladu. Jednotlivý uživatelé nemusí být seznámeni s možností využití datového skladu, nehledě na to, že využití služby datového skladu není z uživatelského hlediska triviální záležitostí a tak je vhodné aktivně nabízet datový sklad jako službu již s předem připravenými vhodnými usecasy. Díky aktivnímu nabízení služby je tak možné implementovat DWH na větším množství dat. Jednotlivé procesní fáze jsou vyobrazeny na \ref{fig:active_offer} a popsány níže. 
        \begin{itemize}
            \item V prvotní fázi aktivní propagace služby datového skladu, dochází ke kontaktování možných nových uživatelů. Děje se tak například za pomoci interního oběžníku, nebo přímým kontaktem fakult a pracovišť a zodpovědných osob.
            \item Obsahem obecného oběžníku je popis samotné služby datového skladu a již fungujících usecasů, které jsou na něm postaveny.
            \item V případě přímého kontaktu je kromě výše uvedeného součástí i konkrétní aplikace s popisem jejího přínosu pro dané pracoviště s případnou referencí na podobný usecase, který je již v podobném prostředí nasazen.
            \item Pokud dojde k vytvoření zájmu, dojde k zahájení procesu, který je totožný se standardním procesem \emph{Žádosti o nový usecase}, s tím rozdílem, že zde je přeskočena první fáze kontaktu a přechází se rovnou do fáze specifikace požadovaného usecasu.
        \end{itemize}
Samotný proces aktivního nabízení není v současné době v užívání (?) a tak ještě není zcela odlazen a nemá konkrétní výsledky. 
...
V rámci diplomové práce je prakticky zpracován jeden uživatelský report založený na datech pocházejících z Informačního sys
tému pro evidenci projektů na Masarykově univerzitě, zkráceně ISEP, který je určen k evidenci projektů na Masarykově univerzitě. Vzhledem k citlivé povaze dat však zde realizovaný a prezentovaný usecase vychází pouze z datového modelu a samotná data jsou náhodně vygenerovaná a nereflektují reálný stav. V případě využití reportu v reálném provozu je možné jej přepojit na reálná data. 

Realizovaný dashboard se věnuje vizualizaci projektových dat na základě genderu osob zainteresovaných do jednotlivých projektů. V rámci přípravy realizace je nutné definovat následující základní parametry dashboardu.

Prvním parametrem je účel samotného dashboardu, kterým je poskytnutí snadného nástroje pro monitoring genderové vyváženosti projektů na Masarykově univerzitě. Pro tento účel je třeba zvolit sledované metriky, které slouží jako hlavní KPI dashboardu, z těchto metriky pak vyplývají i příslušné dimenze dat. Pro splnění účelu dashboard byly zvoleny následující metriky: 
\begin{compactitem}
    \item Úspěšnost návrhů na nové projekty dle genderu navrhovatelů - vizualizace genderového rozdělení projektů, které přešli z fáze návrhu do fáze realizace v komparaci s těmi, které zůstali ve fázi realizace.
    \item Úspěšnost realizovaných projektů dle genderu jejich zpracovatelů - vizualizace relativního poměru úspěšné realizace.
\end{compactitem}
vyjma hlavních KPI, má dashboard obsahovat i následující vedlejší KPI:
\begin{compactitem}
    \item Poměr můžu a žen v rámci všech zpracovávaných projektů - vizualizace poměrného genderového rozdělení v rámci všech realizovaných projektů v průběhu posledních pěti let.
    \item Velikost rozpočtu v závislosti na genderovém složení. 
\end{compactitem}

Posledním základním atributem je pak skupina uživatelů, která bude daný dashboard využívat, v tomto případě se jedná primárně o management MUNI, ale vzhledem k anonymní podstatě dat a jejich společenskému charakteru by měl být výsledný report dostupný i veřejně. 

Z hlediska charakteru dashboardem zpracovávaných dat, u kterých z principu nedochází k časté aktualizace, se bude jednat o strategický dashboard. Data budou aktualizována klasickým ETL procesem datové skladu, který je prováděn jednou denně. 
...
Zdrojová data z ISEP je třeba pro využití v navrhovaném dahsboardu zpracovat za pomoci ETL procesu. O jednotlivé fáze procesu se stará samotná služba datového skladu a jsou rozepsány níže a to v rozšířené verzi, tak jak je ETL proces definován v knize \citetitle{Kimballc2004}.
...
\paragraph{Doručování} - v polsední fází dochází k transformaci dat do datové hvězdy a její uložení. Všechny transformace, kterou jsou s daty provedeny jsou vedeny následující podkapitole.
